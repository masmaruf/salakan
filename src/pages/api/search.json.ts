...
export async function GET() {
  ...
  const searchIndex: SearchRecord[] = [
    ...
    ...programs.filter((p) => p.entry?.statusPublikasi === 'publish').map((p) => ({ title: p.entry.judul, description: p.entry.ringkasan || '', url: `/program/${p.slug}`, type: 'program', date: new Date(p.entry.tanggalUpdate).toISOString(), priority: getPriority(p.entry.unggulan), keywords: [p.entry.kategori || '', p.entry.statusProgram || '', tahapProgramLabels[p.entry.tahapProgram], p.entry.lokasi || '', p.entry.periode || '', p.entry.targetHasilSingkat || '', markdownToText(p.entry.deskripsi), ...(p.entry.tag || [])].filter(Boolean) })),
    ...logs.filter((log) => log.entry?.statusPublikasi === 'publish').map((log) => ({ title: log.entry.judul, description: markdownToText(log.entry.ringkasan) || '', url: `/log-kegiatan?kategori=${encodeURIComponent(log.entry.kategori)}`, type: 'log', date: new Date(log.entry.tanggal).toISOString(), priority: getPriority(false), keywords: [kategoriLogLabels[log.entry.kategori], log.entry.lokasi || '', log.entry.pihakTerlibat || '', markdownToText(log.entry.hasilTindakLanjut), ...(log.entry.tag || [])].filter(Boolean) })),
    ...lembagaItems.filter((item) => item.entry?.statusPublikasi === 'publish').map((item) => ({ title: item.entry.jabatan, description: item.entry.ringkasan || markdownToText(item.entry.deskripsiTugas) || '', url: `/data/lembaga#${item.slug}`, type: 'lembaga', priority: getPriority(item.entry.bidang === 'pimpinan'), keywords: [item.entry.namaPejabat || '', item.entry.bidang || '', item.entry.kontak || '', item.entry.jadwalRutin || '', markdownToText(item.entry.deskripsiTugas), ...(item.entry.fokusKegiatan || []), ...(item.entry.layananUtama || []), ...(item.entry.dokumentasiTerkait || []).map((doc) => doc.label)].filter(Boolean) })),
