...
import { getFokusLembagaLabel, getTagUmumLabel, kategoriLogLabels, statusLayananUmkmLabels, tahapProgramLabels } from '../../lib/content-taxonomy';
...
export async function GET() {
  ...
  const searchIndex: SearchRecord[] = [
    ...news.filter((n) => n.entry?.statusPublikasi === 'publish').map((n) => ({ title: n.entry.judul, description: n.entry.ringkasan || '', url: `/berita/${n.slug}`, type: 'berita', date: new Date(n.entry.tanggal).toISOString(), priority: getPriority(n.entry.unggulan), keywords: [n.entry.kategori || '', ...(n.entry.tag || []), ...((n.entry.tag || []).map((tag) => getTagUmumLabel(tag)))].filter(Boolean) })),
    ...documents.filter((d) => d.entry?.statusPublikasi === 'publish').map((d) => ({ title: d.entry.judul, description: d.entry.ringkasan || '', url: `/data/dokumen/${d.slug}`, type: 'dokumen', date: new Date(d.entry.tanggalUpdate).toISOString(), priority: getPriority(d.entry.unggulan), keywords: [d.entry.kategori || '', markdownToText(d.entry.deskripsi), 'arsip publik'].filter(Boolean) })),
    ...
    ...agendas.filter((a) => a.entry?.statusPublikasi === 'publish').map((a) => ({ title: a.entry.judul, description: a.entry.ringkasan || '', url: `/agenda/${a.slug}`, type: 'agenda', date: new Date(a.entry.tanggal).toISOString(), priority: getPriority(a.entry.unggulan), keywords: [a.entry.label || '', ...(a.entry.tag || []), ...((a.entry.tag || []).map((tag) => getTagUmumLabel(tag))), a.entry.lokasi || '', a.entry.waktuMulai || ''].filter(Boolean) })),
    ...programs.filter((p) => p.entry?.statusPublikasi === 'publish').map((p) => ({ title: p.entry.judul, description: p.entry.ringkasan || '', url: `/program/${p.slug}`, type: 'program', date: new Date(p.entry.tanggalUpdate).toISOString(), priority: getPriority(p.entry.unggulan), keywords: [p.entry.kategori || '', p.entry.statusProgram || '', tahapProgramLabels[p.entry.tahapProgram], p.entry.lokasi || '', p.entry.periode || '', p.entry.targetHasilSingkat || '', markdownToText(p.entry.deskripsi), ...(p.entry.tag || []), ...((p.entry.tag || []).map((tag) => getTagUmumLabel(tag)))].filter(Boolean) })),
    ...logs.filter((log) => log.entry?.statusPublikasi === 'publish').map((log) => ({ title: log.entry.judul, description: markdownToText(log.entry.ringkasan) || '', url: `/log-kegiatan/${log.slug}`, type: 'log', date: new Date(log.entry.tanggal).toISOString(), priority: getPriority(false), keywords: [kategoriLogLabels[log.entry.kategori], log.entry.lokasi || '', log.entry.pihakTerlibat || '', markdownToText(log.entry.hasilTindakLanjut), ...(log.entry.tag || []), ...((log.entry.tag || []).map((tag) => getTagUmumLabel(tag)))].filter(Boolean) })),
    ...lembagaItems.filter((item) => item.entry?.statusPublikasi === 'publish').map((item) => ({ title: item.entry.jabatan, description: item.entry.ringkasan || markdownToText(item.entry.deskripsiTugas) || '', url: `/data/lembaga#${item.slug}`, type: 'lembaga', priority: getPriority(item.entry.bidang === 'pimpinan'), keywords: [item.entry.namaPejabat || '', item.entry.bidang || '', item.entry.kontak || '', item.entry.jadwalRutin || '', markdownToText(item.entry.deskripsiTugas), ...(item.entry.fokusKegiatan || []), ...((item.entry.fokusKegiatan || []).map((fokus) => getFokusLembagaLabel(fokus))), ...(item.entry.layananUtama || []), ...(item.entry.dokumentasiTerkait || []).map((doc) => doc.label)].filter(Boolean) })),
