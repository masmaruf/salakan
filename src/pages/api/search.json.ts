import { getAgenda, getDokumen, getInventarisWarga, getKasRt, getKegiatan, getLayananWarga, getLogKegiatan, getProgram, getStrukturOrganisasi } from '../../lib/content';
import { kategoriLogLabels } from '../../lib/content-taxonomy';
...
export async function GET() {
  const [agendas, documents, inventory, cashReports, news, services, programs, logs, lembagaItems] = await Promise.all([
    getAgenda(),
    getDokumen(),
    getInventarisWarga(),
    getKasRt(),
  ...
  const searchIndex: SearchRecord[] = [
    ...
    ...programs.filter((p) => p.entry?.statusPublikasi === 'publish').map((p) => ({ title: p.entry.judul, description: p.entry.ringkasan || '', url: `/program/${p.slug}`, type: 'program', date: new Date(p.entry.tanggalUpdate).toISOString(), priority: getPriority(p.entry.unggulan), keywords: [p.entry.kategori || '', p.entry.statusProgram || '', p.entry.lokasi || '', p.entry.periode || '', ...(p.entry.tag || [])].filter(Boolean) })),
