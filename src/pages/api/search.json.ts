import { getAgenda, getDokumen, getInventarisWarga, getKasRt, getKegiatan, getLayananWarga, getPengumuman, getProgram } from '../../lib/content';
import type { SearchRecord } from '../../lib/search';

export async function GET() {
  const [agendas, documents, inventory, cashReports, news, services, announcements, programs] = await Promise.all([
    getAgenda(),
    getDokumen(),
    getInventarisWarga(),
    getKasRt(),
  ...
  const searchIndex: SearchRecord[] = [
    ...
    ...announcements.filter((a) => a.entry?.statusPublikasi === 'publish').map((a) => ({ title: a.entry.judul, description: a.entry.ringkasan || '', url: '/agenda', type: 'pengumuman', date: new Date(a.entry.tanggal).toISOString(), priority: a.entry.unggulan ? 'high' : 'normal', keywords: ['informasi warga', 'pengumuman penting'] })),
    ...
    ...agendas.filter((a) => a.entry?.statusPublikasi === 'publish').map((a) => ({ title: a.entry.judul, description: a.entry.ringkasan || '', url: `/agenda/${a.slug}`, type: 'agenda', date: new Date(a.entry.tanggal).toISOString(), priority: a.entry.unggulan ? 'high' : 'normal', keywords: [a.entry.lokasi || '', a.entry.waktuMulai || ''].filter(Boolean) })),
